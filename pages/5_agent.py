...
tab_local, tab_remote = st.tabs(["Local (Claude Desktop)", "Remote (SSE / HTTP)"])
...
with tab_remote:
    st.markdown("Start the SSE server:")
    st.code("python -m mglia.mcp_server --transport sse --port 8502", language="bash")
    st.caption("Remote SSE transport requires `mcp>=1.0.0`. Not available until the server is deployed.")
...
with st.expander("get_perturbation(gene, model)", expanded=True):
    ...
    with col_l:
        ...
        st.markdown(
            ...
            "functional readouts, and assumption audit flags."
        )
            ...
            json.dumps(
                {
                    ...
                    "assumption_audit": {"low_cell_count_warning": False, "flags": []},
                },
                indent=2,
            ),
            language="json",
...
with st.expander("get_split(task)"):
    col_l, col_r = st.columns([1, 1])
    with col_l:
        st.markdown("**Description**")
        st.markdown(
...
with st.expander("evaluate_predictions(predictions, task)"):
    col_l, col_r = st.columns([1, 1])
    with col_l:
        st.markdown("**Description**")
        st.markdown(
...
st.caption(
    ...
    "Requires data/perturbations.json to be generated (`make generate`)."
)
...
if st.button("Fetch record"):
    with st.spinner("Loading record..."):
        try:
            ...
            st.error(
                "data/perturbations.json not found. Run `make generate` first."
            )
